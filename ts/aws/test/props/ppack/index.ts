...
const stackPolicy: policy.StackValidationPolicy = {
    name: "eks-test",
    description: "EKS integration policy.",
    ...
    validateStack: async (args, reportViolation) => {
        ...
        if (cluster.version !== "1.23") {
            reportViolation(`Expected EKS Cluster '${cluster.name}' version to be '1.23' but found '${cluster.version}'`);
        }
...
const tests = new policy.PolicyPack("policy-pack", {
